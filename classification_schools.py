...
data = dd.read_parquet("full_agg.parquet", engine="pyarrow")
...
data = data[(data['num_tix'].notnull())&(data['temp'].notnull()) & (data['humidity'].notnull())& (data['Schools'].notnull())& (data['Business'].notnull())& (data['event'].notnull())]


# def categorize_tix(n):
#     if n < 4000:
#         return 1
#     if n >=4000 and n < 8000:
#         return 2
#     if n >=8000 and n < 10000:
#         return 3
#     if n >= 10000:
#         return 4


#data['num_tix'] = data['num_tix'].astype(int)
data['num_tix'] = data.apply(lambda row: int(row['num_tix']>9000), axis=1, meta=('num', 'bool'))
#data['num_tix'] = data.apply(lambda row: categorize_tix(int(row['num_tix'])), axis=1, meta=('num', 'int'))


from collections import Counter
print(Counter(data['num_tix']))
...
#data['school'] = data.apply(lambda row: int(row['Schools'])>2, axis=1, meta=('school', 'bool'))
data['Business'] = data['Business'].astype(int)
data['Schools'] = data['Schools'].astype(int)
...
X = data[['day_of_year_sin','day_of_year_cos', 'temp', 'humidity', 'Schools', 'Business']].values.compute()
print("got ")
# X = X.rechunk('auto')
# y = y.rechunk('auto')
